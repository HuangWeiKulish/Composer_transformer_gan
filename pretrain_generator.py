...
time_gen = generator.PretrainGenerator(
    out_notes_pool_size=15002, embed_dim=256, n_heads=4, max_pos=800, time_features=3,
    fc_activation="relu", encoder_layers=2, decoder_layers=2, fc_layers=3, norm_epsilon=1e-6,
    embedding_dropout_rate=0.2, transformer_dropout_rate=0.2, mode_='time')

time_gen.train(dataset, epochs=50, nt_tm_loss_weight=(1, 1), save_model_step=2,
               notes_emb_path=notes_emb_path, notes_gen_path=notes_gen_path, time_gen_path=time_gen_path,
               max_to_keep=5, print_batch=True, print_batch_step=200, print_epoch=True, print_epoch_step=1)
