...
class NotesGenerator(tf.keras.Model):
    ...
    def call(self, x_en_nt, tk, out_seq_len, return_str=False):
...
class TimeGenerator(tf.keras.Model):
    ...
    def call(self, x_en_tm, out_seq_len, vel_norm=64.0, tmps_norm=0.12, dur_norm=1.3, return_denorm=True):
        # x_en_tm: (batch_size, in_seq_len, 3)
        return self.predict_time(x_en=x_en_tm, out_seq_len=out_seq_len, vel_norm=vel_norm, tmps_norm=tmps_norm,
                                 dur_norm=dur_norm, return_denorm=return_denorm)
...
#             encoder_layers=encoder_layers, decoder_layers=decoder_layers, fc_layers=fc_layers,
#             norm_epsilon=norm_epsilon, dropout_rate=transformer_dropout_rate, fc_activation=fc_activation,
#             out_positive=True) if mode_ in ['time', 'both'] else None
#
