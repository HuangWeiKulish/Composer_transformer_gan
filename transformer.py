...
class Embedder(tf.keras.Model):
    ...
    def call(self, x_in):
        # x_in: (time_in, 2): the second dimension contains: notes, duration
    ...
    def embedding(x, notes_pool_size, max_pos, embed_dim, dropout_rate=0.2):
        ...
        # --------------------------- embedding ---------------------------
        notes_emb = Embedding(notes_pool_size, embed_dim)(x_notes)  # (batch_size, time_in, embed_dim)
        pos_emb = Embedder.positional_encoding(max_pos, embed_dim)  # (1, max_pos, embed_dim)
        dur_emb = tf.repeat(Reshape((x.shape[1], 1))(x_duration), repeats=embed_dim, axis=2)  # (batch_size, time_in, embed_dim)

        # --------------------------- combine ---------------------------
        combine = Add()([notes_emb, pos_emb[:, :x.shape[1], :], dur_emb])  # (batch_size, time_in, embed_dim)
...
class Generator(tf.keras.Model):
    ...
    def __init__(self, de_max_pos=50000, embed_dim=256, n_heads=4,
    ...
    def call(self, x_en_melody, x_de_in, mask_padding_en, mask_padding_de, mask_lookahead):
        # x_en  # (in_seq_len, embed_dim): x_en_in is the embed output of notes
        # x_de_in  # (out_seq_len, 2): x_de_in is the un-embedded target
        ...
        x_de_duration = x_de_in[:, 1]  # (batch, out_seq_len, 1)
    ...
    def transformer(x_en, x_de, mask_padding_en, mask_lookahead, mask_padding_de, out_notes_pool_size,
        ...
        # --------------------------- output ---------------------------
        if type == 'melody':
            out = Dense(out_notes_pool_size)(x_de)  # out: (batch, de_time_in, out_notes_pool_size)
        else:  # duration
            out = Dense(1)(x_de)  # out: (batch, de_time_in, 1)
    ...
    def multi_head_self_attention(q, k, v, mask=None, embed_dim=256, n_heads=2, depth=1):
        query = Dense(embed_dim)(q)  # (batch, time_in, embed_dim)
        key = Dense(embed_dim)(k)  # (batch, time_in, embed_dim)
        value = Dense(embed_dim)(v)  # (batch, time_in, embed_dim)

        # split heads
        query = Generator.split_head(query, n_heads, depth)  # (batch, n_heads, length, depth)
        key = Generator.split_head(key, n_heads, depth)  # (batch, n_heads, length, depth)
        value = Generator.split_head(value, n_heads, depth)  # (batch, n_heads, length, depth)

        # self attention
        score = tf.matmul(query, key, transpose_b=True)  # (batch, n_heads, length, length)
        scaled_score = score / tf.math.sqrt(tf.cast(tf.shape(key)[-1], tf.float32))  # (batch, n_heads, length, length)
        if mask is not None:
    ...
    def transformer_decoder_block(x_de, en_out, mask_lookahead, mask_padding, embed_dim=256, n_heads=2, depth=1,
        ...
        attention_2, att_weights_2 = Generator.multi_head_self_attention(
            skip_conn_1, en_out, en_out, mask_padding, embed_dim, n_heads, depth)
...
class GeneratorPretrain(tf.keras.Model):
    ...
    def __init__(self, de_max_pos=50000, embed_dim=256, n_heads=4, out_notes_pool_size=8000, fc_activation="relu",
                 encoder_layers=2, decoder_layers=2, fc_layers=3, norm_epsilon=1e-6, transformer_dropout_rate=0.2,
                 embedding_dropout_rate=0.2, beta_1=0.9, beta_2=0.98, epsilon=1e-9):
    ...
    def train_step(self, x_en_in, x_de_in):
        ...
        with tf.GradientTape() as tape:
            x_en_melody = self.embedder_en(x_en_in)  # (batch, in_seq_len, embed_dim)
            mask_padding_en = util.padding_mask(x_en_in[:, :, 0])  # (batch, 1, 1, seq_len)
            mask_padding_de = util.padding_mask(x_de_in[:, :, 0])  # (batch, 1, 1, seq_len)
            ...
            x_out_melody, all_weights_melody, x_out_duration, all_weights_duration = self.generator(
                x_en_melody, x_de_in, mask_padding_en, mask_padding_de, mask_lookahead)
